...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'%first% %upPage% %prePage%  %linkPage% %downPage% %nextPage% %end%');
    ...
    public function show() {
        ...
        if($this->url){
            ...
            $url    =   rtrim(U('/'.$this->url,$this->parameter,false),$depr).'__PAGE__'.(C('URL_HTML_SUFFIX')?'.'.C('URL_HTML_SUFFIX'):'');
        }else{
        ...
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                ...
                if($this->totalPages != 1){
                    $linkPage .= "<li class='active'><a href='javascirpt:void()'>".$page."</a></li>";
                }
            }
        }
        $pageStr     =   str_replace(
