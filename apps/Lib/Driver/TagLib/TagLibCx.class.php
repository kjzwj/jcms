...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'define'    =>  array('attr'=>'name,value','close'=>0),
    		'for'       =>  array('attr'=>'start,end,name,comparison,step', 'level'=>3),
				'editor'    =>  array('attr'=>'id,name,type,content,width,height'),
        );
    ...
    public function _for($attr, $content){
     ...
     +----------------------------------------------------------
     * editor标签解析 插入可视化编辑器
     * 格式： <editor id="editor" name="remark" type="ueditor" content="{$vo.remark}"></editor>
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     * @param string $attr 标签属性
    ...
    public function _editor($attr)
    {
        ...
        $id				=	!empty($tag['id'])?$tag['id']: 'body';
				$name   	=	!empty($tag['name'])?$tag['name']: 'body';
        $class    = !empty($tag['class'])?$tag['class']: '';
				$content    =   !empty($tag['content'])?$tag['content']: '';
				$width   	=	!empty($tag['width'])?$tag['width']: '820';
				$height   	=	!empty($tag['height'])?$tag['height']: '350';
        $type       =   !empty($tag['type'])?$tag['type']: 'ueditor';
				
				switch($type) {
					 case 'ueditor':
                        $parseStr = '<script id="'.$id.'" name="'.$name.'" type="text/plain" class="'.$class.'">'.$content.'</script><script type="text/javascript" src="__DATA__ueditor/ueditor.config.js"></script><script type="text/javascript" src="__DATA__ueditor/ueditor.all.js"></script><script type="text/javascript">var editor = UE.getEditor("'.$id.'",{"initialFrameWidth":'.$width.',"initialFrameHeight":'.$height.'});</script>';
						//$parseStr  =  '<script type="text/javascript" src="__DATA__ueditor/editor_config.js"></script><script type="text/javascript" src="__DATA__ueditor/editor_all_min.js"></script><link href="__DATA__ueditor/themes/default/css/ueditor.css" rel="stylesheet"><script type="text/plain" id="'.$id.'" class="'.$class.'" name="'.$name.'" >'.$content.'</script><script type="text/javascript">var editor = new baidu.editor.ui.Editor({"initialFrameWidth":'.$width.',"initialFrameHeight":'.$height.'}); editor.render("'.$id.'");</script>';
						break;
						
					default :
							$parseStr  =  '<script type="text/plain" id="'.$id.'" style="'.$style.'" name="'.$name.'" >系统未加载或安装 '.$type.'</script>';
