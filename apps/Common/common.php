...
// 会员头像
function avatar($userid,$size='180'){
    $path = 'uploads/user/';
    $filename = 'user_'.$userid.'x'.$size.'.jpg';
    $file = BASE_PATH.$path.$filename;
    if(is_file($file)){
        return C('sys_siteurl') .'/'. $path . $filename;
    }else{
        return C('sys_siteurl') . '/public/img/default_avatar.png';
    }
}


//取栏目名称
function cate($cat_id,$field='name',$top=0)
{
	$info = D('category')->get_info($cat_id,$top);
	$info[$field];
	return $info[$field];
}
...
function admin_login(){
    ...
    if (empty($user)) {
        ...
        return session('admins_sign') == data_auth_sign($user) ? $user['uid'] : 0;
    }
}
...
function user_login(){
    ...
    if (empty($user)) {
        ...
        return session('user_login_sign') == data_auth_sign($user) ? $user['userid'] : 0;
    }
}
